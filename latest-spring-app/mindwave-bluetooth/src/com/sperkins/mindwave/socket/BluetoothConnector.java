...
public class BluetoothConnector  {
	...
	private static LocalDevice getLocalDevice() throws BluetoothStateException {
		if(null == bluetoothAdapter) {
			...
			LOG.info("Local device address: " + bluetoothAdapter.getBluetoothAddress());
		}
		...
		if(remoteDevices.isEmpty()) {
			...
			synchronized(inquiryCompletedLock) {
	            ...
	            if (started) {
	                ...
	                LOG.info(remoteDevices.size() +  " available devices found");
	            }
			} // end synchronized block
		} else {
			LOG.info(remoteDevices.size() +  " preknown devices found");
		}
	...
	class BluetoothDeviceDiscoveryListener implements DiscoveryListener {
		...
		public void servicesDiscovered(int transId, ServiceRecord[] serviceRecordsArray) {
			LOG.info("Services discovered, transId=" + transId);
			serviceRecords.addAll(Arrays.asList(serviceRecordsArray));
		}
		
		@Override
		public void serviceSearchCompleted(int transId, int responseCode) {
			LOG.info("Search completed, transId=" + transId + ", responseCode=" + responseCode);
			synchronized(serviceSearchCompletedLock) {
		...
		case INQUIRY_ERROR:
			LOG.error("device inquiry ended abnormally");
			break;
		case INQUIRY_TERMINATED:
			LOG.info("Device discovery has been canceled by the application");
			break;
		...
		public void deviceDiscovered(RemoteDevice remoteDevice, DeviceClass deviceClass) {
			try {
				LOG.info("Device discovered: " + remoteDevice.getBluetoothAddress() + "/" + remoteDevice.getFriendlyName(false));
				LOG.info("Device class: " + deviceClass.getMajorDeviceClass() + ":" + deviceClass.getMinorDeviceClass());
				LOG.info("Services classes: " + deviceClass.getServiceClasses());
